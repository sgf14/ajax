...
function setHistGridData(data) {
	...
	for (var i=0; i < data.length; i++) {
		var histData = data[i];
		var tr = $("<tr></tr>");
		tr.append($("<td></td>").html(histData.history_id.value()));
		tr.append($("<td></td>").html(histData));
		tr.append($("<td></td>").html(histData.year));
		tr.append($("<td></td>").html(histData.contents));
		$("#histbody").append(tr);
	}
}
...
function getHistData() {
	...
	$.get("../../data/json/histTableData.json", setHistGridData);
}
